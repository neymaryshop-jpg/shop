...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "shutdown_confirm":
        ...
        await query.edit_message_text(
            "🔄 Выключение системы через 5 минут...\n\n"
            "📍 Магазин включен!\n"
...
async def shutdown_timer():
    """Таймер выключения системы"""
    await asyncio.sleep(300)  # 5 минут
    
    try:
        # Отправляем финальное уведомление
        if monitor:
            await monitor.bot.bot.send_message(
                chat_id=SUPER_ADMIN_ID,
                text="⏰ Время вышло! Начинаю выключение системы..."
            )
        
        # Останавливаем все Docker контейнеры
        subprocess.run("docker-compose down", shell=True, capture_output=True)
        
        # Выключаем систему с паролем
        subprocess.run(f"echo '172839654' | sudo -S shutdown -h now", shell=True, capture_output=True)
...
def main() -> None:
    ...
    # Запускаем проверку здоровья после старта
        ...
        if monitor:
            ...
            if results['overall_status'] == 'healthy':
                await application.bot.send_message(
                    chat_id=SUPER_ADMIN_ID,
                    text="🎉 Магазин включен! 😴 Я иду спать, как закончишь выключи пк"
                )
    
    application.job_queue.run_once(startup_health_check, 30)
